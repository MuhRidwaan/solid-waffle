...
if (isset($_GET['acak'])) {
    ...
    for ($i = 1; $i <= $jumlahsoal; $i++) {
        if (isset($_SESSION['soal' . $i]) && isset($_SESSION['jawaban' . $i])) {
            $dbsoal[$i] = $_SESSION['soal' . $i];
            $dbjawab[$i] = $_SESSION['jawaban' . $i];
            $dbsoalke[$i] = $i;
        } else {
            // Jika variabel sesi tidak tersedia, alihkan pengguna kembali ke halaman utama
...
<body>
    ...
    <div class="container">
        ...
        <div class="col-xs-10 col-xs-offset-2">
            <div class="form-group">
                <a target='_blank' href='hasilsoaljawabfahmil.php?acak=1' class="btn btn-block btn-lg btn-danger"> Lihat
                    Jawaban Paket Soal Fahmil</a>
            </div>
        </div>
