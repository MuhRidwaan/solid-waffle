...
if (isset($_POST['username']) && isset($_POST['password'])) {
    ...
    $sql_check = "SELECT username, password FROM user WHERE username=? AND password=? LIMIT 1";
    ...
    if ($check_log->num_rows == 1) {
        ...
        $check_log->bind_result($returned_username, $returned_password);
    
        while ($check_log->fetch()) {
            $_SESSION['user_login'] = $returned_username;
        }
